...
class DashConfig():
    ...
    def slurp_config_file(self, filename):
        # read iote.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is iote.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        ...
        default_port = 2111 if (network == 'mainnet') else 12111

        # use default port for network if not specified in iote.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from iote.conf
